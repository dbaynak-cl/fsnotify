package fsnotify

import (
	"math"
)

// INT_PHI is for scrambling the keys
const INT_PHI = 0x9E3779B9

// FREE_KEY is the 'free' key
const FREE_KEY = 0

func phiMix(x uint32) uint32 {
	h := x * INT_PHI
	return h ^ (h >> 16)
}

// Map is a map-like data-structure for int64s
type Map struct {
	data       []pathsItem
	fillFactor float64
	threshold  int // we will resize a map once it reaches this size
	size       int

	mask uint32 // mask to calculate the original position

	hasFreeKey bool      // do we have 'free' key in the map?
	freeVal    pathsItem // value of 'free' key
}

func nextPowerOf2(x uint32) uint32 {
	if x == math.MaxUint32 {
		return x
	}

	if x == 0 {
		return 1
	}

	x--
	x |= x >> 1
	x |= x >> 2
	x |= x >> 4
	x |= x >> 8
	x |= x >> 16

	return x + 1
}

func arraySize(exp int, fill float64) int {
	s := nextPowerOf2(uint32(math.Ceil(float64(exp) / fill)))
	if s < 2 {
		s = 2
	}
	return int(s)
}

// New returns a map initialized with n spaces and uses the stated fillFactor.
// The map will grow as needed.
func New(size int, fillFactor float64) *Map {
	if fillFactor <= 0 || fillFactor >= 1 {
		panic("FillFactor must be in (0, 1)")
	}
	if size <= 0 {
		panic("Size must be positive")
	}

	capacity := arraySize(size, fillFactor)
	return &Map{
		data:       make([]pathsItem, capacity),
		fillFactor: fillFactor,
		threshold:  int(math.Floor(float64(capacity) * fillFactor)),
		mask:       uint32(capacity - 1),
	}
}

// Get returns the value if the key is found.
func (m *Map) Get(key uint32) (p pathsItem, b bool) {
	if key == FREE_KEY {
		if m.hasFreeKey {
			return m.freeVal, true
		}
		return p, false
	}

	ptr := phiMix(key) & m.mask
	if ptr < 0 || ptr >= uint32(len(m.data)) { // Check to help to compiler to eliminate a bounds check below.
		return p, false
	}
	k := m.data[ptr].watchId

	if k == FREE_KEY { // end of chain already
		return p, false
	}
	if k == key { // we check FREE prior to this call
		return m.data[ptr], true
	}

	for {
		ptr = (ptr + 1) & m.mask
		k = m.data[ptr].watchId
		if k == FREE_KEY {
			return p, false
		}
		if k == key {
			return m.data[ptr], true
		}
	}
}

// Put adds or updates key with value val.
func (m *Map) Put(val pathsItem) {
	key := val.watchId

	if key == FREE_KEY {
		if !m.hasFreeKey {
			m.size++
		}
		m.hasFreeKey = true
		m.freeVal = val
		return
	}

	ptr := phiMix(key) & m.mask
	k := m.data[ptr].watchId

	if k == FREE_KEY { // end of chain already
		m.data[ptr] = val
		if m.size >= m.threshold {
			m.rehash()
		} else {
			m.size++
		}
		return
	} else if k == key { // overwrite existed value
		m.data[ptr] = val
		return
	}

	for {
		ptr = (ptr + 1) & m.mask
		k = m.data[ptr].watchId

		if k == FREE_KEY {
			m.data[ptr] = val
			if m.size >= m.threshold {
				m.rehash()
			} else {
				m.size++
			}
			return
		} else if k == key {
			m.data[ptr] = val
			return
		}
	}

}

// Del deletes a key and its value.
func (m *Map) Del(key uint32) {
	if key == FREE_KEY {
		m.hasFreeKey = false
		m.size--
		return
	}

	ptr := phiMix(key) & m.mask
	k := m.data[ptr].watchId

	if k == key {
		m.shiftKeys(ptr)
		m.size--
		return
	} else if k == FREE_KEY { // end of chain already
		return
	}

	for {
		ptr = (ptr + 1) & m.mask
		k = m.data[ptr].watchId

		if k == key {
			m.shiftKeys(ptr)
			m.size--
			return
		} else if k == FREE_KEY {
			return
		}

	}
}

func (m *Map) shiftKeys(pos uint32) uint32 {
	// Shift entries with the same hash.
	var last, slot uint32
	var k uint32
	var data = m.data
	for {
		last = pos
		pos = (last + 1) & m.mask
		for {
			k = data[pos].watchId
			if k == FREE_KEY {
				data[last].watchId = FREE_KEY
				return last
			}

			slot = phiMix(k) & m.mask
			if last <= pos {
				if last >= slot || slot > pos {
					break
				}
			} else {
				if last >= slot && slot > pos {
					break
				}
			}
			pos = (pos + 1) & m.mask
		}
		data[last].watchId = k
		m.data[last] = m.data[pos]
	}
}

func (m *Map) rehash() {
	newCapacity := len(m.data) * 2
	m.threshold = int(math.Floor(float64(newCapacity) * m.fillFactor))
	m.mask = uint32(newCapacity - 1)

	data := m.data
	m.data = make([]pathsItem, newCapacity)
	if m.hasFreeKey { // reset size
		m.size = 1
	} else {
		m.size = 0
	}

	var o uint32
	for i := 0; i < len(data); i++ {
		if o != FREE_KEY {
			m.Put(data[i])
		}
	}
}

// Size returns size of the map.
func (m *Map) Size() int {
	return m.size
}

// Keys returns a channel for iterating all keys.
func (m *Map) Keys() chan uint32 {
	c := make(chan uint32, 10)
	go func() {
		data := m.data
		var k uint32

		if m.hasFreeKey {
			c <- FREE_KEY // value is m.freeVal
		}

		for i := 0; i < len(data); i++ {
			k = data[i].watchId
			if k == FREE_KEY {
				continue
			}
			c <- k // value is data[i+1]
		}
		close(c)
	}()
	return c
}
